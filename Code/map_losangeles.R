...
Center_LA <- c(34.05382732728222, -118.24276528846235)

bb_LA <- make_square(Center_LA, 1.5) %>% bb_box()
...
counties_CA <- st_crop(counties_CA,
                       xmin=bb_LA[1,1],xmax=bb_LA[1,2],
                       ymin=bb_LA[2,1],ymax=bb_LA[2,2])
...
ggplot() +
  ...
  geom_sf(data = others$osm_polygons, 
          ...
          #alpha = 0.6,
          colour = NA) +
  coord_sf(xlim = c(bb_LA[1,1], bb_LA[1,2]),
           ylim = c(bb_LA[2,1], bb_LA[2,2]),
           expand = FALSE) +
  theme_void() +
  theme_dark +
...
ggsave('LA.png', width = 7, height = 7, units = 'in')
