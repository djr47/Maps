...
Center_Florence <- c(43.77748031433929, 11.25870557988345)

bb_Florence_square <- make_square(Center_Florence, 1.5) %>% bb_box()

water <- bb_Florence_square %>%
  opq() %>%
...
ggplot() +
  ...
  geom_sf(data = buildings$osm_polygons, 
          ...
          #alpha = 0.6,
          colour = NA) +
  coord_sf(xlim = c(bb_Florence_square[1,1], bb_Florence_square[1,2]),
           ylim = c(bb_Florence_square[2,1], bb_Florence_square[2,2]),
           expand = FALSE) +
  theme_void() +
  theme_dark +
  labs(title = "florence")

ggsave('florence.png', width = 7, height = 7, units = 'in')
