...
Center_Edinburgh <- c(55.9548, -3.1884) # center of london


bb_Edinburgh_square <- make_square(Center_Edinburgh, 1.5) %>% bb_box()
bb_Edinburgh_sq_border <- make_square(Center_Edinburgh, 1.5) %>% bb_box()

water <- bb_Edinburgh_square %>%
  opq() %>%
...
buildings <- bb_Edinburgh_square %>%
  ...
  add_osm_feature(key = 'building',
                  value = c('yes',
                            ...
                            'government')) %>%
  osmdata_sf() #%>%
  #trim_osmdata(bb_Edinburgh_polygon)

islands <- bb_Edinburgh_square %>%
  opq() %>%
  add_osm_feature(key = 'place', 
                  value = 'islet') %>%
  osmdata_sf() 

bridges <- bb_Edinburgh_square %>%
  opq() %>%
...
ggplot() +
  ...
  geom_sf(data = buildings$osm_multipolygons, 
          ...
          #alpha = 0.6,
          colour = NA) +
  coord_sf(xlim = c(bb_Edinburgh_square[1,1], bb_Edinburgh_square[1,2]),
           ylim = c(bb_Edinburgh_square[2,1], bb_Edinburgh_square[2,2]),
           expand = FALSE) +
  theme_void() +
  theme_dark +
