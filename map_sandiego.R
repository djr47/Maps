...
bb_SD <- make_square(Center_SD, 1.5) %>% bb_box()
...
counties_CA <- st_crop(counties_CA,
                       xmin=bb_SD[1,1],xmax=bb_SD[1,2],
                       ymin=bb_SD[2,1],ymax=bb_SD[2,2])
...
ggplot() +
  ...
  geom_sf(data = others$osm_polygons, 
          ...
          #alpha = 0.6,
          colour = NA) +
  coord_sf(xlim = c(bb_SD[1,1], bb_SD[1,2]),
           ylim = c(bb_SD[2,1], bb_SD[2,2]),
           expand = FALSE) +
  theme_void() +
  theme_dark +
